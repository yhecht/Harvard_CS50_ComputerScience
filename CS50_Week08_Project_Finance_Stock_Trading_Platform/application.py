# Yannique Hecht
# HARVARD CS50 Week 8 - Finance - Implement a complex anf fully functional website via which users can “buy” and “sell” stocks
...
# Configure Jinja custom filter to display USD currency
...
def index():
    ...
    # Pass list of lists to template page, template to iterate it to extract data into table
    total = cash
    stocks = []
    for index, row in enumerate(rows):
        stock_info = lookup(row['symbol'])

        # create a list with all information about the stock, append it to a list of every stock owned by the user
...
def buy():
    ...
    # Ensure user has logged in / User reached route via POST (as by submitting a form via POST
    if request.method == "POST":
        # Obtain the data necessary for the transaction
        amount=int(request.form.get("amount"))
        symbol=lookup(request.form.get("symbol"))['symbol']

        # Control if stock symbol is valid
...
@login_required
def history():
    """Show recent transations"""

    # query database with recent transactions history
    rows = db.execute("SELECT * FROM transactions WHERE user_id = :user",
                            user=session["user_id"])

    # pass list of lists to template page, template to iterate to extract data into table
    transactions = []
    for row in rows:
        stock_info = lookup(row['symbol'])

        # create a list with all tinformation about transaction, append to a list of every stock transaction
...
def quote():
    ...
    if request.method == "POST":
        ...
        if not stock:
            return apology("Could not find the requested stock")
